...
public class A2ASpringAutoConfiguration {
    ...
    @ConditionalOnMissingBean
    @ConditionalOnProperty(name = "a2a.server.enabled", havingValue = "true", matchIfMissing = true)
    public A2AServerRoute a2aServerRoute(JSONRPCHandler jsonrpcHandler, AgentCard agentCard) {
        return new A2AServerRoute(jsonrpcHandler, agentCard);
    }

    @Bean
    public A2AServerRouteBuilder a2AServerRouteBuilder() {
        return new A2AServerRouteBuilder();
    }
}
